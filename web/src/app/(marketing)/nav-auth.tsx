  ...
  useEffect(() => {
    ...
    async function swap() {
      ...
      const avatar = avatarUrl
        ? `<span class="glo-um-avatar" style="background-image:url('${escapeHtml(avatarUrl)}')"></span>`
        : `<span class="glo-um-avatar">${initial}</span>`;

      const menu = document.createElement("div");
      menu.className = "glo-usermenu";
      menu.innerHTML = `
        <button class="glo-um-trigger" type="button" aria-haspopup="menu" aria-expanded="false">${avatar}<span class="glo-um-name">${escapeHtml(name)}</span> ▾</button>
